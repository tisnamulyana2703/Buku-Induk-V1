    ...
    <PrintWrapper documentTitle="BUKU INDUK SISWA (DENGAN DESKRIPSI CAPAIAN)">
      <div className="font-sans text-xs space-y-4 text-slate-900">
        ...
        <div className="border-b-2 border-black pb-2 text-center space-y-0.5">
          ...
          <h2 className="font-bold text-sm uppercase">LEMBAR BUKU INDUK SISWA (NILAI & DESKRIPSI CAPAIAN)</h2>
        ...
        <table className="w-full border-collapse border border-black font-sans text-xs">
          <thead>
            <tr className="bg-slate-200 border-b border-black font-bold uppercase text-center">
              ...
              <th className="border border-black p-2 text-left">Deskripsi Capaian Pembelajaran</th>
            </tr>
          </thead>
          <tbody>
            {currentRecord?.grades.map((g, idx) => (
              <tr key={g.code} className="border-b border-black">
                ...
                <td className="border border-black p-2 font-bold text-slate-900">{g.namaMataPelajaran}</td>
                <td className="border border-black p-2 text-center font-bold text-sm">{g.nilaiAkhir}</td>
                <td className="border border-black p-2 text-center font-bold">{g.predikat}</td>
                <td className="border border-black p-2 leading-relaxed text-[11px] text-slate-800">
                  {g.deskripsiCapaian || 'Menunjukkan penguasaan kompetensi yang baik.'}
                </td>
              </tr>
            )) || (
              <tr>
        ...
        {currentRecord && (
          <div className="grid grid-cols-12 gap-4 font-sans text-xs pt-2">
        ...
        {/* Catatan Wali Kelas */}
        {currentRecord?.catatanWaliKelas && (
          <div className="border border-black p-2 font-sans text-xs">
            <span className="font-bold uppercase block mb-0.5">Catatan Wali Kelas:</span>
            <p className="italic text-slate-800 text-[11px]">"{currentRecord.catatanWaliKelas}"</p>
          </div>
        )}
